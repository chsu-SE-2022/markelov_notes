...
Mobile authorization(int number, int& traffic, double& time, Mobile& Network, Queue<string>& q, Queue<string>& error, Queue<int>& q_num, int& correct) {
    Mobile Mobile_Connected(Network);
    q.add(Network.getName());
    q_num.add(number);
    correct++;
    cout << Network;
...
int main() {
    ...
    while (true) {
        ...
        cin >> number;
        count++;
        if (number < 0 || number > 18) {     //если введен номер несуществуещей сети, выводится сообщение об ошибке
            NetworkError(1);
            continue;
        }
