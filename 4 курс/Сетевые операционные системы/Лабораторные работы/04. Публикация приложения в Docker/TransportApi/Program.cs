...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        // Ensure writes are flushed to the main DB file (avoid WAL)
        db.Database.ExecuteSqlRaw("PRAGMA journal_mode=DELETE;");
        db.Database.ExecuteSqlRaw("PRAGMA synchronous=FULL;");
    }
    catch { /* ignore if provider/connection not ready yet */ }
}
...
// Disable HTTPS redirection to allow plain HTTP on configured port
