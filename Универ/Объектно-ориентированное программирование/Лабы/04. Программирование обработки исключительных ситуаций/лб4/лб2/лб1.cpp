...
WiFi authorization(int number, int& traffic, double& time, string password, WiFi& Network, Queue<string>& q, Queue<string>& error, Queue<int>& q_num, int& correct) {
    if (password == Network.getPassword()) {      //если пароль верный, то выводится информация о сети
        ...
        throw invalid_argument("Пароль неверный!");   //если пароль неверный, выводится сообщение об ошибке
...
int main() {
    ...
    while (true) {
        ...
        count++;
        try {
            if (number < 0 || number > 29) throw logic_error("Такой сети (устройства) нет!");   //если введен номер несуществуещей сети/устройства, выводится сообщение об ошибке
        }
        ...
        if (number == 0 && !q.empty()) {
            ...
            cin >> num;
            try {
                if (num < 1 || num > 6) throw out_of_range("Такого варианта нет!");
            }
            ...
            try {
                ...
                case 0: {
                    ...
                    correct--;
                      ...
                      break;
                default: throw runtime_error("Такого сайта нет!");
                    break;
                }
            }
